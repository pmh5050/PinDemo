...
#define DEBUG_MOTOR_INDEX 0

#if DEBUG_MOTOR_INDEX
int DebugMotorIndex = 0;
#endif
...
#define MOTOR_P_GAIN 5
#define MOTOR_I_GAIN 1 / 20
#define MOTOR_D_GAIN 1 / 1000 // 0.3
...
void MotorPositionController(int SelectedMotorIndex, uint32_t DeltaTime)
{
  ...
  {
    Serial.print(ControlTerm);
    Serial.print("(ct), ");
    Serial.print(EncError * MOTOR_P_GAIN);
    Serial.print("(p), ");
    Serial.print(IntErrCount[SelectedMotorIndex] * MOTOR_I_GAIN * DeltaTime);
    Serial.print("(i), ");
  ...
  */
  ...
  IntErrCount[SelectedMotorIndex] = constrain(IntErrCount[SelectedMotorIndex], -300, 300);
...
void setup()
{
  ...
  {
    DesiredEncCount[MotorIndex] = -1000 * (MotorIndex + 1);

  }

#if DEBUG_MOTOR_INDEX
  DebugMotorIndex = 0;
  DesiredEncCount[DebugMotorIndex] = 1000;
#endif
...
void loop()
{
  ...
  //PrintEncCount();
#else
  for (MotorIndex = 0; MotorIndex < 10; MotorIndex++)
  {
